...
class Menu_model extends CI_Model {
    ...
    {
        $this->db->where('status !=', 0); // Tambahkan kondisi untuk memfilter status yang tidak bernilai 0
    ...
    {
        $query = $this->db->query("SELECT * FROM menu WHERE status != 0 ORDER BY kd_menu DESC LIMIT 6");
        return $query->result();
    }
    
    public function tampilFood()
    {
        $query = $this->db->query("SELECT * FROM menu WHERE type = 'makanan' AND status != 0");
        return $query->result();
    }
    ...
    {
        $query = $this->db->query("SELECT * FROM menu WHERE type = 'minuman' AND status != 0");
        return $query->result();
    }
    ...
    {
        $query = $this->db->query("SELECT * FROM menu WHERE type = 'snack' AND status != 0");
        return $query->result();
    }
